...
export default function App() {
  ...
  const [bankAccounts, setBankAccounts] = useState([
    {
      ...
      accountNumber: '1170011106804',
      accountName: 'BENDAHARA RT 08',
    },
  ]);
  ...
  const [selectedYearLaporan, setSelectedYearLaporan] =
    useState(currentYearAuto);

  useEffect(() => {
    const timer = setTimeout(() => setDbLoading(false), 2000);
    ...
    const unsub = onSnapshot(
      ...
      (snapshot) => {
        if (!snapshot.exists()) {
          ...
          if (data.saldoAwalTahun !== undefined)
            setSaldoAwalTahun(data.saldoAwalTahun);
          ...
          if (data.thrBaseAmount !== undefined)
            setThrBaseAmount(data.thrBaseAmount);
        }
        setDbLoading(false);
      },
      (err) => {
  ...
  const laporanData = useMemo(() => {
    ...
    for (let m = 0; m < 12; m++) {
      ...
      residents.forEach((res) => {
        const pay = res.payments[selectedYearLaporan]
          ? res.payments[selectedYearLaporan][m]
          : undefined;
        ...
        <div className="w-full max-w-md bg-slate-50 min-h-screen relative shadow-2xl overflow-hidden flex flex-col items-center justify-center p-6">
          ...
          <div className="absolute inset-0 flex items-center justify-center pointer-events-none opacity-10 z-0">
            <img
              src="/logo-bogor.png"
              alt=""
              className="w-[120%] max-w-none blur-md"
            />
          </div>
          ...
          <img
            src="/logo-bogor.png"
            alt="Logo Bogor"
            className="w-20 h-20 object-contain mb-4 drop-shadow-sm z-10"
            onError={(e) => (e.target.style.display = 'none')}
          />

          <h1 className="text-2xl font-extrabold text-green-700 tracking-tight mb-1 text-center z-10">
            VILLA PERMATA MAS 1
          </h1>
          <p className="text-xs font-bold text-slate-500 uppercase tracking-widest mb-8 text-center z-10">
            Sistem Kas RT 08
          </p>
    ...
    return (
      <div className="space-y-4 pb-12">
        <h2 className="text-lg font-bold text-emerald-800">
          Saldo Pengajian RT 08
        </h2>
        <div className="bg-emerald-50 p-6 rounded-2xl shadow-sm border border-emerald-100">
          <h3 className="text-xs font-bold text-emerald-600 mb-2 uppercase">
            Total Saldo
          </h3>
          {isEditingPengajian && userRole === 'PENGURUS' ? (
            <div className="flex items-center gap-2">
              <input
                type="number"
                value={tempPengajian}
                onChange={(e) => setTempPengajian(e.target.value)}
                className="w-full text-lg font-bold p-2 rounded-xl border border-emerald-200 text-emerald-800 focus:outline-none"
                autoFocus
              />
              <button
              ...
              <button
                onClick={() => setIsEditingPengajian(false)}
                className="bg-slate-300 hover:bg-slate-400 text-slate-700 p-2 rounded-xl"
              >
                <X className="w-5 h-5" />
              </button>
            </div>
            ...
            <div className="text-3xl font-extrabold text-emerald-700 flex items-center justify-between">
              ...
              {userRole === 'PENGURUS' && (
                <button
                  onClick={() => {
                    setIsEditingPengajian(true);
                    setTempPengajian(pengajianData.saldo);
                  }}
                  className="text-emerald-400 hover:text-emerald-600 p-2 bg-emerald-100 rounded-full"
                >
        ...
        <div className="bg-white p-4 rounded-2xl shadow-sm border border-slate-200">
          <h3 className="text-xs font-bold text-slate-500 mb-2 uppercase">
            Input Informasi Manual
          </h3>
          {isEditingInfo && userRole === 'PENGURUS' ? (
            <div className="space-y-2">
              <textarea
                ...
                placeholder="Ketik informasi manual di sini..."
              />
              <div className="flex gap-2">
                ...
                <button
                  ...
                  className="flex-1 py-2 bg-emerald-600 text-white rounded-lg text-sm font-bold flex justify-center items-center gap-2"
                >
              ...
              <div className="text-sm text-slate-700 whitespace-pre-line min-h-[60px] p-3 bg-slate-50 rounded-xl border border-slate-100">
                {pengajianData.info || (
                  <span className="text-slate-400 italic">
                    Klik ikon pensil untuk tambah informasi...
                  </span>
                )}
              </div>
              {userRole === 'PENGURUS' && (
                <button
                  onClick={() => {
                    ...
                    setTempInfo(pengajianData.info || '');
                  }}
                  className="absolute top-2 right-2 text-slate-400 hover:text-emerald-600 p-1"
                >
  ...
  const LaporWargaView = () => {
    ...
    const handleSubmit = (e) => {
      ...
      if (!form.nama || !form.nomor)
        return alert('Nama dan Nomor Rumah wajib diisi!');
      const newLapor = {
        id: Date.now(),
        ...form,
    ...
    return (
      <div className="space-y-4 pb-12">
        ...
        <div className="bg-white p-4 rounded-xl shadow-sm border border-slate-200">
          ...
          <form onSubmit={handleSubmit} className="space-y-3">
            <input
              ...
              className="w-full border border-slate-200 rounded-lg text-sm p-2.5 focus:ring-red-500"
            />
            <input
              type="text"
            ...
            />
            <textarea
              placeholder="Isi Laporan / Pengaduan..."
              required
              value={form.laporan}
              ...
              {form.foto ? (
                <div className="flex flex-col items-center gap-1 text-green-600">
                  <img
                    ...
                    alt="Preview"
                  />
                  <span className="text-xs truncate w-full px-4">
                    {form.fotoName}
                  </span>
                </div>
              ) : (
            ...
            <button
              type="submit"
              className="w-full py-2.5 bg-red-600 text-white rounded-lg text-sm font-bold flex justify-center gap-2 hover:bg-red-700"
            >
              Kirim Laporan
            </button>
          </form>
        ...
        <div className="space-y-3">
          <h3 className="font-bold text-slate-700 text-sm mt-4">
            Riwayat Pengaduan
          </h3>
              ...
              >
                  ...
                  <button
                    onClick={() => {
                      if (window.confirm('Hapus laporan ini?')) {
                        const updated = laporanWarga.filter(
                          (item) => item.id !== l.id
                        );
                        setLaporanWarga(updated);
                        saveLaporanToDatabase(updated);
                    ...
                    className="absolute top-2 right-2 text-slate-300 hover:text-red-500 bg-slate-50 p-1.5 rounded"
                  >
                ...
                {l.foto && (
                  <img
                    src={l.foto}
                    alt="Lampiran"
                    className="mt-2 rounded-lg max-h-40 object-contain border border-slate-100"
                  />
                )}
              </div>
            ))
  ...
  const DashboardView = () => {
    ...
    const startEditBank = () => {
      setTempBanks([...bankAccounts]);
      setIsEditingBank(true);
    };
    const handleAddBank = () => {
      setTempBanks([
        ...tempBanks,
        { id: Date.now(), bankName: '', accountNumber: '', accountName: '' },
      ]);
    };
    const handleRemoveBank = (id) => {
      setTempBanks(tempBanks.filter((b) => b.id !== id));
    };
    const handleBankChange = (id, field, value) => {
      setTempBanks(
        tempBanks.map((b) => (b.id === id ? { ...b, [field]: value } : b))
      );
    };
    const handleSaveBanks = () => {
      setBankAccounts(tempBanks);
      saveToDatabase('bankAccounts', tempBanks);
      ...
      <div className="space-y-4">
        <div className="bg-gradient-to-br from-green-600 to-emerald-800 rounded-2xl p-6 text-white shadow-lg relative overflow-hidden">
          ...
          <h2 className="text-sm font-medium text-green-100 mb-1">
            Saldo Akhir Kas RT 08 (Bulan {MONTHS[currentMonthIdx]})
          </h2>
          {isEditingSaldo && userRole === 'PENGURUS' ? (
            <div className="mb-4 bg-white/10 p-3 rounded-lg border border-green-400/30">
              <label className="text-xs text-green-100 block mb-1">
                Sesuaikan Saldo Kas Saat Ini (Rp):
              </label>
              <div className="flex items-center gap-2">
                <input
                  type="number"
                  ...
                  className="text-slate-800 p-1.5 rounded w-full text-sm font-bold outline-none"
                  autoFocus
                />
                <button
                  onClick={handleSaveSaldo}
                  className="bg-green-500 hover:bg-green-400 text-white p-1.5 rounded transition"
                >
                  <Save className="w-5 h-5" />
                </button>
                <button
                  onClick={() => setIsEditingSaldo(false)}
                  className="bg-red-500 hover:bg-red-400 text-white p-1.5 rounded transition"
                >
                  <X className="w-5 h-5" />
                </button>
              </div>
              ...
              {userRole === 'PENGURUS' && (
                <button
                  onClick={() => {
                    ...
                    setTempSaldo(dataBulanIni.saldoAkhir);
                  }}
                  className="text-green-200 hover:text-white p-1.5 bg-white/10 rounded-full transition"
                  title="Edit Saldo"
                >
                  <Edit3 className="w-4 h-4" />
                </button>
          ...
          <div className="grid grid-cols-2 gap-4 mt-6 border-t border-white/20 pt-4">
            <div>
              ...
              <div className="font-semibold">
                {formatRp(dataBulanIni.penerimaan.total)}
              </div>
            </div>
            <div>
              ...
              <div className="font-semibold">
                {formatRp(
                  dataBulanIni.totalRutin + dataBulanIni.totalTidakRutin
                )}
              </div>
            </div>
          </div>
        ...
        <div className="grid grid-cols-4 gap-3">
          <button
            ...
            className="bg-white p-3 rounded-2xl shadow-sm border border-slate-100 flex flex-col items-center justify-center gap-2 hover:bg-slate-50 transition"
          >
          ...
          <button
            ...
            className="bg-yellow-50 p-3 rounded-2xl shadow-sm border border-yellow-200 flex flex-col items-center justify-center gap-2 hover:bg-yellow-100 transition relative overflow-hidden"
          >
            <div className="bg-yellow-100 p-2.5 rounded-full text-yellow-600">
              <Gift className="w-5 h-5" />
            </div>
            <span className="text-[11px] font-semibold text-yellow-800 text-center leading-tight">
              Iuran THR
            </span>
          </button>
          <button
            onClick={() => setActiveTab('pengajian')}
            className="bg-emerald-50 p-3 rounded-2xl shadow-sm border border-emerald-200 flex flex-col items-center justify-center gap-2 hover:bg-emerald-100 transition relative overflow-hidden"
          >
            <div className="bg-emerald-100 p-2.5 rounded-full text-emerald-600">
              <BookOpen className="w-5 h-5" />
            </div>
            <span className="text-[11px] font-semibold text-emerald-800 text-center leading-tight uppercase">
              Pengajian
            </span>
          </button>
          <button
            onClick={() => setActiveTab('laporWarga')}
            className="bg-red-50 p-3 rounded-2xl shadow-sm border border-red-200 flex flex-col items-center justify-center gap-2 hover:bg-red-100 transition col-span-4 relative overflow-hidden"
          >
            <div className="flex items-center gap-3">
              <div className="bg-red-100 p-2.5 rounded-full text-red-600">
                <Megaphone className="w-5 h-5" />
              </div>
              <span className="text-[12px] font-semibold text-red-800 text-center leading-tight">
                Lapor Warga
              </span>
            </div>
          </button>
          ...
          {isEditingBank ? (
            <div className="space-y-3">
              {tempBanks.map((account) => (
                <div
                  key={account.id}
                  className="flex flex-col gap-2 p-3 bg-slate-50 rounded-xl border border-slate-200 relative"
                >
                  <button
                    onClick={() => handleRemoveBank(account.id)}
                    className="absolute top-2 right-2 text-slate-400 hover:text-red-500 bg-white rounded p-1 shadow-sm"
                  >
                    <Trash2 className="w-4 h-4" />
                  </button>
                  <input
                    type="text"
                    placeholder="Nama Bank (Cth: Mandiri)"
                    value={account.bankName}
                    onChange={(e) =>
                      handleBankChange(account.id, 'bankName', e.target.value)
                    }
                    className="w-full border border-slate-200 rounded-lg text-sm p-2 pr-8 focus:ring-blue-500"
                  />
                  <input
                    type="text"
                    ...
                    className="w-full border border-slate-200 rounded-lg text-sm p-2 focus:ring-blue-500"
                  />
                  <input
                    type="text"
              ...
              <button
                ...
                className="w-full py-2 bg-blue-50 text-blue-600 text-xs font-bold rounded-xl border border-blue-200 flex items-center justify-center gap-1 hover:bg-blue-100 transition"
              >
                <PlusCircle className="w-4 h-4" /> Tambah Rekening
              </button>
              <div className="flex gap-2 pt-2">
                ...
                <button
                  ...
                  className="flex-1 py-2 bg-green-600 text-white rounded-lg text-sm font-bold flex justify-center items-center gap-2 hover:bg-green-700 transition"
                >
                  <Save className="w-4 h-4" /> Simpan
                </button>
              </div>
            ...
            <div className="flex items-start gap-4">
              <div
                ...
                className="w-16 h-16 bg-blue-50 rounded-xl flex items-center justify-center border border-blue-200 shrink-0 cursor-pointer hover:bg-blue-100 transition relative group shadow-sm overflow-hidden mt-1"
              >
                <img
                  src="/qris.jpeg"
                  alt="QR"
                  ...
                  onError={(e) =>
                    (e.target.src =
                      'https://api.qrserver.com/v1/create-qr-code/?size=150x150&data=1170011106804')
                  }
                />
              ...
              <div className="flex-1 space-y-3">
                {bankAccounts.length === 0 ? (
                  ...
                  bankAccounts.map((account) => (
                    <div
                      key={account.id}
                      className="space-y-1 border-b border-slate-100 pb-2 last:border-0 last:pb-0"
                    >
  ...
  const IuranView = () => {
    ...
    const filteredWarga = residents.filter(
      (r) =>
        r.name.toLowerCase().includes(searchIuran.toLowerCase()) ||
        r.block.toLowerCase().includes(searchIuran.toLowerCase())
    );
      ...
      <div className="h-[75vh] flex flex-col space-y-3">
            ...
            {userRole === 'PENGURUS' && (
              <button
                onClick={() => {
                  ...
                  setShowWargaForm(true);
                }}
                className="bg-green-600 text-white px-3 py-2 rounded-lg text-xs font-bold flex items-center gap-1 hover:bg-green-700"
              >
                <PlusCircle className="w-3.5 h-3.5" /> Warga
              </button>
            )}
            <select
              value={selectedYearIuran}
              onChange={(e) => setSelectedYearIuran(Number(e.target.value))}
        ...
        {showWargaForm && userRole === 'PENGURUS' && (
          <div className="bg-white p-4 rounded-xl shadow-sm border border-green-200 shrink-0 mb-1 animate-in fade-in slide-in-from-top-2">
            <h3 className="font-bold text-slate-700 mb-3">
              {wargaForm.id ? 'Edit Data Warga' : 'Tambah Warga Baru'}
            </h3>
            <form onSubmit={handleSaveWarga} className="space-y-3">
              ...
              <input
                type="text"
                placeholder="Blok / No Rumah..."
                required
                value={wargaForm.block}
                onChange={(e) =>
                  setWargaForm({
                    ...wargaForm,
              ...
              <input
                type="number"
                placeholder="Iuran Pokok per Bulan..."
                required
                value={wargaForm.defaultAmount}
                onChange={(e) =>
                  setWargaForm({ ...wargaForm, defaultAmount: e.target.value })
                }
                className="w-full border border-slate-200 rounded-lg text-sm p-2"
              />
        ...
        <div className="bg-blue-50 p-3 rounded-xl border border-blue-100 shrink-0">
          <div className="flex justify-between items-center mb-2">
            <h3 className="text-xs font-bold text-blue-800 flex items-center gap-1">
              ...
              {rincian.pokok.toLocaleString('id-ID')})
            </h3>
            {userRole === 'PENGURUS' && !isEditingRincian && (
              <button
                onClick={() => setIsEditingRincian(true)}
            ...
            {isEditingRincian && (
              <div className="flex gap-2">
                ...
                <button
                  onClick={() => setIsEditingRincian(false)}
                  className="bg-slate-300 text-slate-700 p-1 rounded"
                >
                  <X className="w-3 h-3" />
                </button>
              </div>
          ...
          {isEditingRincian ? (
            <div className="grid grid-cols-2 gap-2 text-[10px] text-blue-800">
              <div className="flex items-center justify-between gap-1">
                ...
                <input
                  ...
                  value={rincian.pokok}
                  onChange={(e) =>
                    setRincian({ ...rincian, pokok: Number(e.target.value) })
                  }
                  className="w-16 p-0.5 rounded border focus:outline-none"
                />
              </div>
              <div className="flex items-center justify-between gap-1">
                <span>a. Sampah:</span>
                <input
                  type="number"
                  value={rincian.sampah}
                  onChange={(e) =>
                    setRincian({ ...rincian, sampah: Number(e.target.value) })
                  }
                  className="w-16 p-0.5 rounded border focus:outline-none"
                />
              </div>
              <div className="flex items-center justify-between gap-1">
                <span>b. Keamanan:</span>
                <input
                  type="number"
                  value={rincian.keamanan}
                  onChange={(e) =>
                    setRincian({ ...rincian, keamanan: Number(e.target.value) })
                  }
                  className="w-16 p-0.5 rounded border focus:outline-none"
                />
              </div>
              <div className="flex items-center justify-between gap-1">
                ...
                <input
                  ...
                  value={rincian.kasRw}
                  onChange={(e) =>
                    setRincian({ ...rincian, kasRw: Number(e.target.value) })
                  }
                  className="w-16 p-0.5 rounded border focus:outline-none"
                />
              </div>
              <div className="flex items-center justify-between gap-1">
                ...
                <input
                  ...
                  value={rincian.kasRt}
                  onChange={(e) =>
                    setRincian({ ...rincian, kasRt: Number(e.target.value) })
                  }
                  className="w-16 p-0.5 rounded border focus:outline-none"
                />
              </div>
              <div className="flex items-center justify-between gap-1">
                ...
                <input
                  type="number"
                  value={rincian.sosial}
                  onChange={(e) =>
                    setRincian({ ...rincian, sosial: Number(e.target.value) })
                  }
                  className="w-16 p-0.5 rounded border focus:outline-none"
                />
              </div>
              <div className="flex items-center justify-between gap-1">
                <span>f. Posyandu:</span>
                <input
                  type="number"
                  value={rincian.posyandu}
                  onChange={(e) =>
                    setRincian({ ...rincian, posyandu: Number(e.target.value) })
                  }
                  className="w-16 p-0.5 rounded border focus:outline-none"
                />
              </div>
            ...
            <div className="grid grid-cols-2 gap-x-2 gap-y-1 text-[10px] text-blue-700">
              ...
              <p>
                b. Uang Keamanan: Rp{rincian.keamanan.toLocaleString('id-ID')}
              </p>
        ...
        <div className="relative shrink-0">
          ...
          <input
            ...
            value={searchIuran}
            onChange={(e) => setSearchIuran(e.target.value)}
          />
        </div>
          ...
          <div className="overflow-auto flex-1">
            <table className="w-full text-xs text-left">
              <thead className="bg-slate-50 text-slate-600">
                <tr>
                  <th className="sticky top-0 left-0 bg-slate-100 p-3 z-30 shadow-sm whitespace-nowrap">
                    Nama Warga
                  </th>
                  {MONTHS.map((m, i) => (
                    <th
                      key={i}
                      className="sticky top-0 bg-slate-50 p-3 text-center min-w-[50px] font-semibold z-20 shadow-sm"
                    >
                      {m.slice(0, 3)}
                    </th>
                  ))}
              ...
              <tbody className="divide-y divide-slate-100">
                {filteredWarga.map((warga) => (
                  <tr key={warga.id} className="hover:bg-slate-50 transition">
                            ...
                            <button
                              onClick={(e) => {
                                ...
                                setWargaForm({
                                  ...
                                  name: warga.name,
                                  block: warga.block,
                                  defaultAmount: warga.defaultAmount,
                                });
                                setShowWargaForm(true);
                              }}
                    ...
                    {MONTHS.map((m, i) => {
                      const paymentsYear =
                        warga.payments[selectedYearIuran] || {};
                      const isPaid =
                        paymentsYear[i] ||
              ...
              <tfoot className="bg-green-50 font-semibold relative z-20">
                <tr>
                  <td className="sticky bottom-0 left-0 bg-green-100 p-3 z-30 shadow-sm text-green-800 text-[11px] whitespace-nowrap">
                    Total Terkumpul
                  </td>
                  {MONTHS.map((_, i) => {
                    ...
                    return (
                      <td
                        key={i}
                        className="sticky bottom-0 z-20 bg-green-50 p-2 text-center border-l border-green-100 text-[10px] text-green-700 whitespace-nowrap shadow-sm"
                      >
                        {totalMth > 0
  ...
  const ThrView = () => {
    ...
    const saveThr = (residentId) => {
      ...
      const newResidents = residents.map((r) => {
        if (r.id === residentId) {
          ...
          return {
            ...r,
            thrPayments: { ...currentThr, [selectedYearThr]: amount },
          };
        }
        return r;
      });
    ...
    const filteredWarga = residents.filter(
      (r) =>
        r.name.toLowerCase().includes(searchThr.toLowerCase()) ||
        r.block.toLowerCase().includes(searchThr.toLowerCase())
    );
    ...
    return (
      <div className="h-[75vh] flex flex-col space-y-3">
        <div className="flex items-center justify-between shrink-0">
            ...
            {isEditingBaseThr && userRole === 'PENGURUS' ? (
              <div className="flex items-center gap-2 mt-1">
                <input
                  ...
                  autoFocus
                  value={tempBaseThr}
                  onChange={(e) => setTempBaseThr(Number(e.target.value))}
                  className="w-24 p-1 text-xs font-bold text-slate-800 border border-yellow-400 rounded focus:outline-none"
                />
                <button
                  onClick={handleSaveBaseThr}
              ...
              <div className="flex items-center gap-2 mt-0.5">
                <p className="text-xs text-yellow-600">
                  Rp {thrBaseAmount.toLocaleString('id-ID')} / Warga
                </p>
                {userRole === 'PENGURUS' && (
                  <button
                    onClick={() => {
                      setIsEditingBaseThr(true);
                      setTempBaseThr(thrBaseAmount);
                    }}
          ...
          <select
            value={selectedYearThr}
            onChange={(e) => setSelectedYearThr(Number(e.target.value))}
        ...
        <div className="relative shrink-0">
          ...
          <input
            ...
            placeholder="Cari nama warga atau blok..."
            value={searchThr}
            onChange={(e) => setSearchThr(e.target.value)}
          />
        </div>
        <div className="flex-1 min-h-0 bg-white rounded-xl shadow-sm border border-slate-200 flex flex-col relative overflow-hidden">
          <div className="overflow-auto flex-1">
                    ...
                    {editingThrId === warga.id && userRole === 'PENGURUS' ? (
                      <div className="flex items-center gap-1">
                        <input
                          ...
                          autoFocus
                          value={tempThrAmount}
                          onChange={(e) => setTempThrAmount(e.target.value)}
                          className="w-20 p-1.5 text-xs font-bold text-slate-800 border border-yellow-400 rounded focus:outline-none focus:ring-2 focus:ring-yellow-500 bg-white"
                          placeholder="Nominal"
                        />
                        <button
                          onClick={() => saveThr(warga.id)}
                          className="bg-yellow-500 hover:bg-yellow-600 text-white p-1.5 rounded"
                        >
                          <Save className="w-4 h-4" />
                        </button>
                        <button
                          onClick={() => deleteThr(warga.id)}
                          className="bg-red-500 hover:bg-red-600 text-white p-1.5 rounded"
                        >
                          <Trash2 className="w-4 h-4" />
                        </button>
                        <button
                          onClick={() => setEditingThrId(null)}
                          className="bg-slate-300 hover:bg-slate-400 text-slate-700 p-1.5 rounded"
                        >
                          <X className="w-4 h-4" />
                        </button>
                      </div>
                      ...
                      <div
                        ...
                        onClick={() => {
                          if (userRole === 'PENGURUS') {
                            setEditingThrId(warga.id);
                            setTempThrAmount(
                              isPaid ? paidAmount : thrBaseAmount
                            );
                          }
                        }}
                      >
                        {isPaid ? (
                          <div className="text-right">
                            <span className="text-[10px] font-bold text-yellow-700 bg-yellow-100 px-2 py-1 rounded inline-block mb-1">
                              LUNAS
                            </span>
    ...
    return (
      <div className="space-y-4">
        <div className="flex items-center justify-between mb-2">
          ...
          <select
            className="bg-white border border-slate-300 text-slate-700 text-sm rounded-lg block p-2"
            value={selectedMth}
        ...
        {!showForm && userRole === 'PENGURUS' && (
          <button
            onClick={() => {
              ...
              setTDesc('');
              setTType('out');
              setTCategory('Gaji Petugas Keamanan');
            }}
      ...
      <div className="space-y-4 pb-12">
        <div className="flex items-center justify-between mb-4">
          <div>
            ...
            <p className="text-xs text-slate-500">
              Rekap Tahun {selectedYearLaporan} (Geser kanan)
            </p>
          </div>
          <div className="flex items-center gap-2">
            <select
              value={selectedYearLaporan}
              onChange={(e) => setSelectedYearLaporan(Number(e.target.value))}
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-xs text-left text-slate-600">
              <thead className="bg-slate-100 text-slate-700 uppercase">
                <tr>
                  <th className="sticky left-0 bg-slate-100 p-3 z-10 shadow-sm whitespace-nowrap min-w-[200px]">
                    KETERANGAN
                  </th>
                  {MONTHS.map((m) => (
                    <th
              ...
              <tbody className="whitespace-nowrap">
                <tr className="bg-slate-50 font-bold border-b border-slate-200">
                  <td className="sticky left-0 bg-slate-50 p-2 z-10 shadow-sm">
                    (II) Penerimaan
                  </td>
                  <td colSpan={12}></td>
                </tr>
                <tr className="border-b border-slate-100">
                  <td className="sticky left-0 bg-white p-2 pl-6 z-10 shadow-sm">
                ...
                <tr className="border-b border-slate-200 bg-green-50 font-semibold text-green-800">
                  <td className="sticky left-0 bg-green-50 p-2 text-right pr-4 z-10 shadow-sm">
                    Jumlah (II)
                  </td>
                  {laporanData.map((d, i) => (
                    <td key={i} className="px-3 py-2 text-right">
                ...
                <tr className="border-b border-slate-200 bg-red-50 font-semibold text-red-700">
                  <td className="sticky left-0 bg-red-50 p-2 text-right pr-4 z-10 shadow-sm">
                    Jumlah (III)
                  </td>
                  {laporanData.map((d, i) => (
                    <td key={i} className="px-3 py-2 text-right">
                ...
                <tr className="border-b border-slate-200 bg-orange-50 font-semibold text-orange-700">
                  <td className="sticky left-0 bg-orange-50 p-2 text-right pr-4 z-10 shadow-sm">
                    Jumlah (IV)
                  </td>
                  {laporanData.map((d, i) => (
                    <td key={i} className="px-3 py-2 text-right">
                ...
                <tr className="bg-slate-100 font-bold border-b border-slate-300">
                  <td className="sticky left-0 bg-slate-100 p-3 z-10 shadow-sm uppercase">
                    (V) (Defisit) / Surplus
                  </td>
                  {laporanData.map((d, i) => (
                    <td
        ...
        <div className="pt-4 text-center">
          <p className="text-[10px] text-slate-400 font-bold tracking-widest uppercase">
            © 2026 RT 08 - VILLA PERMATA MAS 1
          </p>
        </div>
      </div>
    );
    ...
    const handleSaveAsset = (e) => {
      ...
      if (assetForm.id) {
        newAssets = assets.map((a) =>
          a.id === assetForm.id
            ? { ...a, name: assetForm.name, count: assetForm.count }
            : a
        );
      } else {
        newAssets = [
          ...assets,
          { id: Date.now(), name: assetForm.name, count: assetForm.count },
        ];
      }
      setAssets(newAssets);
      ...
      <div className="space-y-6 pb-12">
        <div>
          <h2 className="text-lg font-bold text-slate-800 mb-1">
            Informasi & Kegiatan RT 08
          </h2>
          {!showForm && userRole === 'PENGURUS' && (
            <button
              onClick={() => setShowForm(true)}
              className="w-full bg-blue-50 text-blue-600 border border-blue-200 p-3 rounded-xl font-bold flex items-center justify-center gap-2 shadow-sm mb-4"
            >
                ...
                <div className="border border-dashed border-slate-300 rounded-lg p-3 text-center relative hover:bg-slate-50 transition mt-2">
                  ...
                  {infoData.fileData ? (
                    <div className="flex flex-col items-center justify-center gap-2 text-green-600">
                      {infoData.fileType?.includes('image') ||
                      infoData.fileName.match(/\.(jpg|jpeg|png|gif)$/i) ? (
                        <img
                          src={infoData.fileData}
                          alt="Preview"
              ...
              >
                ...
                {agenda.fileData ? (
                  <div className="mt-2 border border-slate-200 rounded-lg overflow-hidden bg-slate-50">
                    {agenda.fileType?.includes('image') ||
                    agenda.fileName?.match(/\.(jpg|jpeg|png|gif)$/i) ? (
                      ...
                      />
                    ) : agenda.fileType?.includes('pdf') ||
                      agenda.fileName?.toLowerCase().endsWith('.pdf') ? (
                      <iframe
                        src={agenda.fileData}
                        className="w-full h-64 border-none"
                        title={agenda.fileName}
                      />
                    ) : (
        ...
        <div>
          <h2 className="text-lg font-bold text-slate-800 mb-1 mt-8">
            Inventaris Warga RT 08
          </h2>
          <button
            onClick={() => setShowAset(!showAset)}
            className="w-full bg-white border border-slate-200 p-4 rounded-xl shadow-sm flex items-center justify-between"
          >
            {' '}
            <div className="flex items-center gap-3">
            ...
            <div className="mt-3">
              {userRole === 'PENGURUS' && !showAsetForm && (
                <button
                  ...
                  className="w-full mb-3 bg-green-50 text-green-700 border border-green-200 p-2 rounded-xl text-sm font-bold flex items-center justify-center gap-2"
                >
                ...
                <div className="bg-white border border-green-200 p-4 rounded-xl shadow-sm mb-3">
                  <form onSubmit={handleSaveAsset} className="space-y-3">
                    ...
                    <input
                      ...
                      onChange={(e) =>
                        setAssetForm({ ...assetForm, count: e.target.value })
                      }
                      className="w-full border rounded-lg text-sm p-2"
                    />
                    <div className="flex gap-2">
                      <button
                        ...
                        className="flex-1 py-1.5 bg-slate-100 text-slate-600 rounded-lg text-sm font-bold"
                      >
                        Batal
                      </button>{' '}
                      <button
                        type="submit"
                        className="flex-1 py-1.5 bg-green-600 text-white rounded-lg text-sm font-bold"
                      >
                        Simpan
                      </button>
                    </div>
      ...
      <div className="w-full max-w-md bg-slate-50 min-h-screen relative shadow-2xl overflow-hidden flex flex-col">
        ...
        <div className="absolute inset-0 flex items-center justify-center pointer-events-none opacity-10 z-0">
          <img
            src="/logo-bogor.png"
            alt=""
            className="w-[150%] max-w-none blur-sm"
          />
        </div>
            ...
            <div className="bg-white p-6 rounded-3xl shadow-2xl w-full max-w-xs relative flex flex-col items-center">
              ...
              <div className="w-64 h-64 bg-blue-50 border-2 border-blue-200 rounded-2xl flex items-center justify-center p-2 mb-6 shadow-inner relative overflow-hidden">
                <img
                  ...
                  onError={(e) =>
                    (e.target.src = `https://api.qrserver.com/v1/create-qr-code/?size=400x400&data=${
                      bankAccounts.length > 0
              ...
              <p className="text-[11px] text-center text-slate-500 px-4">
                Scan kode QR di atas untuk pembayaran via m-banking atau
                e-wallet.
              </p>
            </div>
          </div>
